...
public class ImageController {
    ...
    @RequestMapping("/images/comment")
    public String newComment() {
        return "images";
    }

    @RequestMapping(value = "/image/{imageId}/{imageTitle}/comments", method = RequestMethod.POST)
    public String uploadComment(@RequestParam("comment") String comment, @PathVariable("imageId") Integer id, comment newComment, HttpSession session,Model model) throws IOException {
        User user = (User) session.getAttribute("loggeduser");
        newComment.setUser(user);
        newComment.setDate(new Date());
        Image image = imageService.getImage(id);
        newComment.setImg(image);
        newComment.setText(comment);
        imageService.uploadComment(newComment);
        Image image2 = imageService.getImage(id);
        model.addAttribute("image", image2);
        model.addAttribute("tags",image2.getTags());
        model.addAttribute("user", image2.getUser());
        model.addAttribute("comments", image2.getComments());
        return "images/image";
    }
    ...
    public String editImage(@RequestParam("imageId") Integer imageId,@RequestParam("profileId") Integer profileId, Model model, HttpSession session) {
        ...
        if(id==profileId){
            ...
            model.addAttribute("tags",image.getTags());
            model.addAttribute("user", image.getUser());
            model.addAttribute("error_e", true);
            return "images/image";
        }
    }
    ...
    public String deleteImageSubmit(@RequestParam(name = "imageId") Integer imageId,@RequestParam("profileId") Integer profileId,HttpSession session, Model model) {
        User user = (User) session.getAttribute("loggeduser");
        Integer id =user.getId();
        if(id==profileId){
            ...
            model.addAttribute("tags",image.getTags());
            model.addAttribute("user", image.getUser());
            model.addAttribute("error_d", true);
            return "images/image";
        }
    }
